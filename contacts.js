...
const removeContact = async (contactId) => {
    try {
        ...
        // work similar to the find function in the getContactById but we need the index to delete the contact
        const index = contacts.findIndex((item) => item.id === contactId);
        if (index === -1) {
            return null;
        }
        const deletedContact = contacts.splice(index, 1) // we are passing the  index to the splice funtion to delete the specified contactId
        await fs.writeFile(contactsPath, JSON.stringify(contacts, null, 2)); // we are stringifying the object before appending it back to t he contacts.json
